...
subtitle: "Regional, County, and Sewer Level COVID Wastewater Concentration"
output:
  html_document:
    df_print: paged
---
...
cat(subtitle)
```
```{r Plot region, echo=FALSE, fig.height=4, fig.width=8, message=FALSE, warning=FALSE}
 
plot(region)
```

```{r Display caption info, echo=FALSE, warning=FALSE, comment=NA}
...
plot(counties)
```
```{r Caption for county plot, echo=FALSE, message=FALSE, warning=FALSE, comment=NA}
...
```{r Systems plot, echo=FALSE, message=FALSE, warning=FALSE}
plot(systems)
```

```{r Caption for systems plot, echo=FALSE, message=TRUE, warning=TRUE, comment=NA}
