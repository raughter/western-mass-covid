...
```
**Support this project here:** [https://ko-fi.com/raughter]. **Thank you!**<br>

 
 <font size="5">Regional Concentration and Wastewater Viral Activity Level (WVA)</font>
...
```
```{r Plot counties, echo=FALSE, fig.height=6.25, fig.width=10, message=FALSE, warning=FALSE, fig.cap = sprintf("The above figure shows county-level wastewater COVID concentration for %d sewer systems in Berkshire, Hampden, Hampshire, and Franklin counties. Wastewater Viral Activity (WVA) levels are relative to the previous %d months of data. Blue annotations show the date of most recent report and the current WVA level.", sewer_num, time_months)}
plot(county_plot)
```
...
```
 ...
 <font size="5">Individual Sewer System Concentration and Wastewater Viral Activity Level</font>
```{r Systems plot, echo=FALSE, message=FALSE, warning=FALSE, fig.height=7, fig.width=10, fig.cap = sprintf("The above figure shows reported concentration data from %d individual sewer systems in Berkshire, Hampden, Hampshire, and Franklin counties. Wastewater Viral Activity (WVA) levels are relative to the previous %d months of data. Blue annotations show the date of most recent report and the current WVA level.", sewer_num, time_months)}
plot(systems_plot)
```
**Support this project here:** [https://ko-fi.com/raughter]. **Thank you!**
<br><br>
...
• Wastewater Viral Activity levels are based on the CDC methodology linked below, with two differences: (1) use of weighted statistics (median and standard deviation) to calculate thresholds, with population served and reporting interval as weights; and (2) The baseline is updated each time the WVA is calculated, rather than every 6 months.<br> 
