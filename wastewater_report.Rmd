...
```
```{r Plot counties, echo=FALSE, fig.height=6.25, fig.width=10, message=FALSE, warning=FALSE, fig.cap = sprintf("The above figure shows county-level wastewater COVID concentration and Wastewater Viral Activity level (WVA) for %d sewer systems in Berkshire County (Pittsfield); Hampden County (Springfield); Hampshire County (Amherst, Northampton), and Franklin County (Greenfield, Montague, S. Deerfield, Sunderland). Concentrations below 1 copies/mL are cropped out to make the figures easier to read. WVA levels are relative to the previous %d months of data. Blue annotations show the date of most recent report and the current WVA level.", sewer_num, time_months)}
plot(county_plot)
```
...
# loop through smooth_groups
for (i in seq_along(smooth_groups$group)){
  tryCatch({
    # create smooths
    temp <- smooth_operator(
      full_sample %>% 
        filter(date > max(date) - years(time_years)) %>% 
        filter(smooth_group == smooth_groups$group[i]), 
      time_years = 1
    )
    
    # add new smooth data to output
    ...
    tempnote <- data.frame(
      ...
      note = paste0(
        ...
        "\nWVA score: ",  
        temp %>% arrange(date) %>% pull(wva_score) %>% last() %>% as.character()
      )
    )
    summ_temp <- rbind(summ_temp, tempnote)
    
  }, error = function(e) {
    failed_groups <<- c(failed_groups, smooth_groups$group[i])
    message("Skipping group ", smooth_groups$group[i], ": ", e$message)
  })
}

# optional: print summary at the end
if (length(failed_groups) > 0) {
  message("\nFailed groups (", length(failed_groups), "): ", 
...
plot_title_syst <- paste0("wma_wastewater_", max(dfreg$date), "_systems", ".tiff")
width = 1992*1.25
height = 1200*1.25

ggsave(systems_plot, path = here("plots/"), filename = plot_title_syst,
       width = width, height = height, units = "px", bg="white")
...
```{r Systems plot, echo=FALSE, message=FALSE, warning=FALSE, fig.height=7, fig.width=10, fig.cap = sprintf("The above figure shows reported concentration data and Wastewater Viral Activity level (WVA) from %d individual sewer systems in Berkshire, Hampden, Hampshire, and Franklin counties. Concentrations below 1 copies/mL are cropped out to make the figures easier to read. WVA levels are relative to the previous %d months of data. Blue annotations show the date of most recent report and the current WVA level.", sewer_num, time_months)}
plot(systems_plot)
```
...
<br><br>
**If you have the means, you can buy me a coffee here** https://ko-fi.com/raughter. <br>
**You can find my writing here** https://cultureofhomecoming.substack.com/ <br>
**Stay safe, y'all <3**
